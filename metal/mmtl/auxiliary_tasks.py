...
def add_spacy_ner_labels(payload):
    ...
    def get_spacy_ner_tags(it):
        ...
        sent1_token_map, sent1_bert = get_bert_spacy_index(sent1, tokenizer, add_cls=True)     
        sent2_token_map, sent2_bert = get_bert_spacy_index(sent2, tokenizer, add_cls=False)
        
        spacy_ner_tags = np.zeros((len(sent1_bert+sent2_bert),))
        
        # This condition should be true if we've done things correctly!
        bert_tokens_orig = payload.data_loader.dataset.bert_tokenizer.convert_ids_to_tokens(payload.data_loader.dataset.bert_tokens[it])
        ...
        for ent in sent1.ents:
            sent_1_tags[sent1_token_map[ent.start]:sent1_token_map[ent.end]]=ent.label_
        for ent in sent2.ents:
            sent_2_tags[sent2_token_map[ent.start]:sent2_token_map[ent.end]]=ent.label_
